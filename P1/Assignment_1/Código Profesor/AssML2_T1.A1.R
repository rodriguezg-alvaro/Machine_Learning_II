...
#################################################################################
#
frectALL = read.table("SimDataRECTANGLEwithoutNoise.dat", sep = "", header = TRUE, stringsAsFactors = FALSE)
frectALL$Y = as.factor(frectALL$Y)
str(frectALL)
summary(frectALL)
ggplot(frectALL)+geom_point(aes(x=X1,y=X2,color=Y))

frectNoiseALL = read.table("SimDataRECTANGLEwithNoise.dat", sep = "", header = TRUE, stringsAsFactors = FALSE)
